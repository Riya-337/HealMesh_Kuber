...
export default function ActionPanel({ action, namespace }: { action: ParsedAction; namespace: string }) {
      ...
      {action.action_type === 'NONE' && (
        <div className="space-y-2">
          <p className="text-xs text-white/60 leading-relaxed">
            External dependency failure (PostgreSQL connection refused).<br />
            No safe automated action — operator intervention required.
          </p>
          <div
            className="flex items-center gap-2 p-2.5 rounded-lg text-xs text-white/40"
            style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}
          >
            <Lock size={12} className="flex-shrink-0" />
            No executor action — NONE returned by parser
          </div>
        </div>
      )}

      {isProtected && action.action_type !== 'NONE' && (
        <div className="flex items-center gap-2 p-2.5 rounded-lg text-xs text-hm-rust"
          style={{ background: 'rgba(193,123,58,0.08)', border: '1px solid rgba(193,123,58,0.20)' }}>
          <Lock size={12} />
          Protected namespace — execution blocked
        </div>
      )}

      {action.action_type !== 'NONE' && !isProtected && action.params && (
        <div
